...
$script:Version = "1.0"
$script:TempPath = Join-Path $env:TEMP "ArtIsWar-Setup"
$script:BoxWidth = 70
    ...
    while ($true) {
        ...
        switch ($key.ToLower()) {
            's' {
                ...
                if (Test-Path $scExe) { Start-Process $scExe }
                else { Write-Host "$($script:BoxMargin)LEQ Control Panel not found." -ForegroundColor Yellow }
...
function Start-ParallelDownloads {
    ...
    while (@($state.Values | Where-Object { $_ -ne 'done' -and $_ -ne 'error' }).Count -gt 0) {
            ...
            if ($uj.Handle.IsCompleted) {
                ...
                if ($spec.Method -eq 'BITS') {
                    ...
                    try {
                        ...
                        $state[$key] = 'error'
                        $errors[$key] = "BITS start failed: $_"
                    }
                } else {
        ...
        foreach ($key in @($started.Keys)) {
            ...
            elseif ($state[$key] -eq 'downloading' -and $elapsed -gt 120) {
                ...
                $state[$key] = 'error'
                $errors[$key] = 'Download timed out (120s)'
            }
        }
    ...
    if ($errors.Count -gt 0) {
        $msg = ($errors.GetEnumerator() | ForEach-Object { "$($_.Key): $($_.Value)" }) -join "; "
...
function Get-Downloads {
    ...
    $specs += @{
        ...
        Method      = 'IWR'
        Url         = 'https://artiswar.io/tools/leq'
        BaseUrl     = $null
        UrlResolver = $null
    }
    ...
    if ($specs.Count -gt 0) {
        $dlResults = Start-ParallelDownloads -Specs $specs -TotalCount $totalComponents
    }
...
try {
    ...
    if ($hwChoice -eq 3) {
        ...
        if ($deviceChoice -eq 'soundblaster') {
            $result = Write-DeviceCompletion -IncludeCreativeApp
        } else {
            $result = Write-DeviceCompletion
        }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBwm+M/8ikHEKdN
# UCVZZKsrWbrfvxMeNSC8kslR9lddg6CCJXYwggVvMIIEV6ADAgECAhBI/JO0YFWU
...
# CSqGSIb3DQEJBDEiBCDhAfdQB1+2N2n+tdvSIyweU4wZ0dWpicd/sF+KCWEhpzAN
# BgkqhkiG9w0BAQEFAASCAgAqP8ic/EbJS4Ag4lbqwttchbfUr0uKQvavxM3xGIjN
# pEjMTbVSL1UXG8xI4I3sSjzAV28I9JHJvD/5FSBqsvPjU5IsuWpZfh9JvFE+kpp4
# 4Qh5btfg+FxYfkk/m1jlASwuoYf1BeJaE96Dx15zQqrgKbpHu3NvGmvJ50iWuvMo
# nMqmUWhI7Nq2GH+HkcSK2zTfeTdVzluDeY3QZ/sYByKZArcjXfqpPVGTSub48aE/
# u+Yl9OdHerYvAfwXuPIPKlvbRqV25kak2tPwRCJBadOSt1oRsvKclUUZk5wo0/+v
